...
class CaseMeta:
    ...
    def from_questionnaire(
        ...
        seeds_raw = ask(f"How many random seeds? [default {base.n_seeds}]: ").strip()
    ...
    def as_prompt_block(self) -> str:
        ...
        if self.n_seeds and self.n_seeds > 1:
            lines.append(f"- {self.n_seeds} random seeds were run.")
...
def intent_prompt_block(intent: NarrationIntent) -> str:
    ...
    framing = {
        ...
        NarrationIntent.DIAGNOSE: (
            ...
            "(e.g., healthy archetype mix + high INVESTIGATE → unconverged, "
            "not unhealthy) and recommend the next action (more training, "
            "regularisation, feature audit) rather than a structural change."
        ),
        NarrationIntent.PRUNE: (
            ...
            "NOT safe to prune until stable) and the cosens group analysis. "
            "Do NOT recommend pruning if trust_instability_high fires; say so."
        ),
